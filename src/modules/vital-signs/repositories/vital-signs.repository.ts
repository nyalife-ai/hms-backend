/**
 * File: vital-signs.repository.ts
...
import { PrismaVitalSignRepository } from './prisma/prisma-vital-sign.repository';
...
export const VitalSignRepositoryProvider: Provider = {
  provide: VITAL_SIGNS_REPOSITORY,
  inject: [ConfigService, PrismaService],
  useFactory: (config: ConfigService, prisma: PrismaService) => {
    if (resolveOrmType(config) === 'prisma') {
      return new PrismaVitalSignRepository(prisma);
    }
