...
import { PrismaRadiologyRepository } from './prisma/prisma-radiology.repository';
...
export const RadiologyRepositoryProvider: Provider = {
  provide: RADIOLOGY_REPOSITORY,
  inject: [ConfigService, PrismaService],
  useFactory: (config: ConfigService, prisma: PrismaService) => {
    if (resolveOrmType(config) === 'prisma') {
      return new PrismaRadiologyRepository(prisma);
    }
