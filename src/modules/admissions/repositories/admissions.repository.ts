...
import { PrismaAdmissionRepository } from './prisma/prisma-admission.repository';
...
export const AdmissionRepositoryProvider: Provider = {
  provide: ADMISSIONS_REPOSITORY,
  inject: [ConfigService, PrismaService],
  useFactory: (config: ConfigService, prisma: PrismaService) => {
    if (resolveOrmType(config) === 'prisma') {
      return new PrismaAdmissionRepository(prisma);
    }
