...
import { PrismaFollowUpRepository } from './prisma/prisma-follow-up.repository';
...
export const FollowUpRepositoryProvider: Provider = {
  provide: FOLLOW_UPS_REPOSITORY,
  inject: [ConfigService, PrismaService],
  useFactory: (config: ConfigService, prisma: PrismaService) => {
    if (resolveOrmType(config) === 'prisma') {
      return new PrismaFollowUpRepository(prisma);
    }
