...
describe('P0 Bull job-type registry', () => {
  ...
  it('NotificationsProcessor exposes a handler method per notification job', () => {
    ...
    for (const [job, method] of Object.entries(methods)) {
      ...
      expect(
        typeof (NotificationsProcessor.prototype as Record<string, unknown>)[
          method
        ],
      ).toBe('function');
    }
...
describe('P0 BillingStkProcessor unknown job handling', () => {
  it('onFailed downgrades foreign job names (session.create) to ignore warn', () => {
    const checkout = {
  ...
  it('onFailed still reports real STK failures', () => {
    ...
    processor.onFailed(
      { id: '1', name: BILLING_STK_JOB, attemptsMade: 2 } as never,
      new Error('M-Pesa timeout'),
    );

    const msg = String(warn.mock.calls[0]?.[0] ?? '');
    expect(msg).toMatch(/STK job failed/);
